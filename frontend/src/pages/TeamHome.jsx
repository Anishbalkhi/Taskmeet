...
import { Plus, List, Grid3x3, Search, Circle, Users as UsersIcon, ChevronDown, Check, Lock, Trash2, Filter } from "lucide-react";
...
const TeamHome = () => {
    ...
    const canAddTask = usePermission("createTask");
                    ...
                    <div className="flex items-center justify-between mb-4 gap-3">
                        <div className="min-w-0">
                            <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                                <UsersIcon className="w-5 h-5 sm:w-6 sm:h-6 shrink-0" />
                                <span className="truncate">Team Tasks</span>
                            </h1>
                            <p className="text-xs sm:text-sm mt-0.5 truncate" style={{ color: 'var(--text-muted)' }}>
                                Collaborate in {currentWorkspace?.name || "this workspace"}
                            </p>
                        </div>
                        ...
                        {canAddTask ? (
                            <button
                                ...
                                className="px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5 shrink-0"
                            ...
                            <div
                                className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg text-sm font-medium border border-dashed shrink-0"
                    ...
                    >
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="pl-3 pr-7 py-1.5 text-sm rounded-lg appearance-none cursor-pointer focus:outline-none"
                                style={{
                ...
                {taskLoading ? (
                    ...
                    <div className="rounded-xl p-10 sm:p-14 text-center" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
                        <div className="w-14 h-14 rounded-full mx-auto mb-4 flex items-center justify-center" style={{ background: 'var(--bg-hover)' }}>
                            <UsersIcon className="w-6 h-6" style={{ color: 'var(--text-muted)' }} />
                        </div>
                        <p className="font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                            {allTasks.length === 0 ? "No tasks yet in this workspace" : "No tasks match the current filters"}
                        </p>
                        <p className="text-sm mb-5" style={{ color: 'var(--text-muted)' }}>
                            {allTasks.length === 0 ? "Create the first task to get started" : "Try adjusting your filters"}
                        </p>
                                ...
                                >
                                    ...
                                    <div className="hidden sm:grid grid-cols-12 gap-3 px-4 py-3 items-center">
                                        ...
                                        <div className="col-span-2 flex items-center">
                                            {(() => {
                                                const user = task.assignedTo;
                                                if (!user?.id) return <span className="text-sm" style={{ color: 'var(--text-muted)' }}>—</span>;
                                                return (
                                                    ...
                                                    >
                                                        {statusOptions.map(opt => (
                                                            <button
                                                                ...
                                                                style={{ color: 'var(--text-secondary)', background: task.status === opt.value ? 'var(--bg-hover)' : 'transparent' }}
                                                                onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredTasks.map((task, index) => (
                            ...
                            >
                                ...
                                <div className="mb-3">
                                    {(() => {
                                        const user = task.assignedTo;
                                        if (!user?.id) return <span className="text-xs" style={{ color: 'var(--text-muted)' }}>No assignee</span>;
                                        return (
                        ...
                        {canAddTask && (
                            <button
                                onClick={() => setIsModalOpen(true)}
                                className="rounded-xl p-4 flex flex-col items-center justify-center min-h-[160px] sm:min-h-[200px] transition-all"
                                style={{
